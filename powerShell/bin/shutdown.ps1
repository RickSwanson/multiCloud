...
$doit = read-host "Are you sure you want to power down vm's using "search" as your search?  Enter yes to continue or test to just list them"
...
function getthem {
    ...
    foreach ($vm in $vms) {
        write-host $vm.Name "is targeted for shutdown"
        if ($doit -eq "yes") {
...
if ($doit -eq "yes" -or $doit -eq "test") {
    ...
    $output = "\\baz-filer01\repo\changes\downVMs-$targetVC-$today.csv"
    $cred = if ($cred) {
        $cred
    } else {
        Get-Credential
